...
class BaseSimulation
  ...
  def send_data(data)
    ...
    top_layer_index.downto(1) do |index| # Send the data down the layers
      @stack[index - 1].receive_from_next_upper_layer(@stack[index].send_to_next_lower_layer(data)) # Send the data to the next lower layer
    end
  ...
  def receive_data(data)
    ...
    0.upto(top_layer_index - 1) do |index|  # iterate through the layers
      @stack[index + 1].receive_from_next_lower_layer(@stack[index].send_to_next_upper_layer(data)) # send the data to the layer above
      @stack[6].send_to_next_upper_layer(data) # Return the data from the top layer
    end
    @stack[top_layer_index].send_to_next_lower_layer(data) # Return the data from the top layer
